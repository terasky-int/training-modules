 ...
 */
 ...
 locals {
  # Extract slack_gcp_secret for each notification channel (taking the first one found)
  slack_secrets = {
    for k, v in var.budget_notification_channels : k => [
      for sl in coalesce(v.sensitive_labels, []) : sl.slack_gcp_secret
      if sl.slack_gcp_secret != null
    ][0]
    if length([
      for sl in coalesce(v.sensitive_labels, []) : sl.slack_gcp_secret
      if sl.slack_gcp_secret != null
...
resource "google_monitoring_notification_channel" "default" {
  ...
  dynamic "sensitive_labels" {
    ...
    content {
      auth_token = sensitive_labels.value.slack_gcp_secret != null ? (
        data.google_secret_manager_secret_version.slack_token[each.key].secret_data
      ) : sensitive_labels.value.auth_token
