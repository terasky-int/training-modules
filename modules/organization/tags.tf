locals {
  ...
  tag_iam = {
    for t in local.tag_iam_temp : "${t.tag}:${t.role}" => t
  }
  ...
  tag_value_iam = {
    for v in local.tag_value_iam_temp : "${v.key}:${v.role}" => v
  }
  ...
  tag_values = {
    for v in local.tag_values_temp : v.key => v
  }
